    ...
    if (!mutex.isLocked()) {
      ...
      try {
        ...
        if (refreshResult.data && isTokens(refreshResult.data)) {
          localStorage.setItem(AUTH_STORAGE_KEY.accessToken, refreshResult.data.accessToken);
          localStorage.setItem(AUTH_STORAGE_KEY.refreshToken, refreshResult.data.refreshToken);
